---
title: "Flow in the city, Fluid in the solid"
year: 2025
category: "School Project"
cover: "/images/project-02/cover.jpg"
images:
  - "/images/project-02/cover.jpg"
  - "/images/project-02/image-1.jpg"
  - "/images/project-02/image-2.jpg"
  - "/images/project-02/image-3.jpg"
  - "/images/project-02/image-4.jpg"
  - "/images/project-02/image-5.jpg"
  - "/images/project-02/image-6.jpg"
featured: true
---

專案概述

"Flow in the City" provides urban residents with an agri-food space where they can engage through education, consumption, and participation. Located within an urban redevelopment zone, the design's fluid spatial quality offers users a sensory experience that transcends the typical city atmosphere.

Site Location: Beitun District, Taichung

Keyword: Curve, Tensile structure, Agri-food space

Tools: Rhino, Grasshopper, AutoCAD, D5 render, Photoshop, Illustrator

設計理念：Leaving a light, flowing presence in the city

Seeking to reclaim a spatial experience where landscape and architecture breathe as one. I hope to create a fluid and free space within this highly planned redevelopment zone.

Urban (Solid) vs Nature (Fluid)

Urban lines and sensory experiences are predominantly defined by rigid verticals and horizontals — seen in building facades, bridges, and walls.

In contrast, nature's rivers and mountains possess organic, dynamic lines and fluid spatial qualities.

空間配置與分析

景觀與植栽策略

Inspired by the area's self-sufficient urban farms, aiming to showcase a diverse landscape of edible plants on this site.

Herbaceous Crops: 30cm

Shrub and Tree Crops: 80cm

Woody Crops: 120cm

Spatial Overview

While the urban grid of the rezoned area is rigid and ordered, human activity and spatial experience should remain free-flowing.

關鍵空間元素：

Membrane and tensile structure roof

RC Load-bearing Wall System

Dig and Fill Earthwork

Skybridge connecting to library & 1F Herbaceous Plant Area

2F Exhibition Space showing farming culture

1.5F Shrub Plant Area and the ramp

1F Farm-to-Table Restaurant & Outdoor Dining Area

B1 Underground Plaza & Bookstore